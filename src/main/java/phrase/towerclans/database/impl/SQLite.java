...
import phrase.towerclans.serializable.LocationSerializable;
...
public class SQLite implements Database {
    ...
    public SQLite(TowerClans plugin, DatabaseManager databaseManager) {
        this.plugin = plugin;
    ...
    public void createTable() {
        ...
        try (Connection connection = databaseManager.getConnection();
            ...
            statement.executeUpdate("CREATE TABLE IF NOT EXISTS towerclans_clan (clan VARCHAR(" + maxSizeClanName + ") PRIMARY KEY, lvl INTEGER, xp INTEGER, bal INTEGER, pvp INTEGER, home TEXT, storage TEXT, colorKey VARCHAR(8))");
    ...
    public void saveClan(ClanImpl clan) {
        CompletableFuture<Void> cf = CompletableFuture.runAsync(() -> {
            try (Connection connection = databaseManager.getConnection();
                 PreparedStatement preparedStatement = connection.prepareStatement("INSERT OR REPLACE INTO towerclans_clan (clan, lvl, xp, bal, pvp, home, storage, colorKey) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public void savePlayer(ModifiedPlayer modifiedPlayer) {
        ...
        CompletableFuture<Void> cf = CompletableFuture.runAsync(() -> {
            try (Connection connection = databaseManager.getConnection();
                 PreparedStatement preparedStatement = connection.prepareStatement("INSERT OR REPLACE INTO towerclans_player (player, stats, permission, clan, rank) VALUES (?, ?, ?, ?, ?)")) {
    ...
    public void loadPlayers() {
        try (Connection connection = databaseManager.getConnection();
            ...
            while (resultSet.next()) {
                ...
                permissionManager.getPlayers().put((Bukkit.getPlayer(playerUUID) == null) ? Bukkit.getOfflinePlayer(playerUUID).getUniqueId() : playerUUID, new Permission(permissionTypes, 0));
